...
const Experience = () => {
  ...
  /*
  - target: containerRef: Specifies the containerRef DOM element as the scroll target.
  - offset: ["start 95%", "end end"]: Defines when the scroll progress begins and ends relative to the viewport:
    "start 95%": Scroll progress starts when the top of the containerRef is 95% of the way down the viewport.
    "end end": Scroll progress ends when the bottom of containerRef is fully within the viewport
   */
      ...
      >
        {experienceData.map((data, i) => (
          ...
          >
            ...
            <div
              className={`w-14 absolute top-20 border border-gray-300 rounded-full aspect-square grid place-items-center text-red-400 font-light translate-y-1/2 bg-white z-10 ${
                i % 2 === 0
        ...
        <motion.div
          initial={{ scaleY: scrollYProgress }}
          style={{ scaleY: scrollYProgress }}
          className="absolute w-1 h-full rounded-full bg-gray-300 origin-top" // (origin-top to start anim from botton to top)
        ></motion.div>
      </div>
    </div>
  );
