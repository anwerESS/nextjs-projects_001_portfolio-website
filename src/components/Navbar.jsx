      ...
      <div className="flex flex-col gap-y-3 sm:gap-y-2">
        {navbarData.map((item, i) => (
          ...
          >
            ...
            <span
              className={`text-[10px] tracking-wide -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 text-center ${i % 2 === 0 ? "translate-x-2" : "-translate-x-2"} ${item.id === id && "-translate-x-0 opacity-100"}`}
            >
              {item.name}
            </span>
          </a>
